...
| R-36 | Không viết thẳng lên `main`. Mọi việc đi trên nhánh `<loại>/<mã>-<mô tả>`, vào `main` bằng **merge có review**. Gộp giữ được trailer `Prompt:` của **mọi** prompt trong nhánh: một prompt thì squash, nhiều prompt (tính năng + FIX cho prompt khác) thì `--no-ff`. **Ngoại lệ hẹp:** nhật ký quản trị — thêm/đóng dòng `DEBT.md`, file phán quyết `docs/reviews/*` — được commit thẳng lên `main` vì không phải mã, không qua cổng nào; mọi thứ khác (kể cả `changes/*.md`, tài liệu trong mã) vẫn phải đi nhánh. |
| R-36b | Khối trailer là **đoạn cuối cùng** của thân commit: có **một dòng trống ngay trước** nó và **không** dòng trống nào **bên trong** (`Prompt:`, `Fix:`, `Co-Authored-By:` sát nhau). Sai một trong hai thì `git log --format='%(trailers:key=Prompt)'` trả rỗng và mất đường tra chủ sở hữu. Kiểm sau khi commit: `git log -1 --format='%(trailers:key=Prompt,valueonly)'` phải in ra mã prompt. |
