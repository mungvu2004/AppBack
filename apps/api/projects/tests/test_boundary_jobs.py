"""Ranh giới nhập của `apps.api.projects.jobs` (BE-00 §12, B2-01 [12]): tiến trình Python mới."""
...
def test_jobs_imports_without_web_or_crypto_packages() -> None:
    """Worker nhập được `apps.api.projects.jobs` khi `fastapi`, `starlette`, `jwt`, `argon2` bị chặn."""
    blocked = "; ".join(f"sys.modules[{name!r}] = None" for name in WORKER_BLOCKED)
    result = _python(f"import sys; {blocked}; import apps.api.projects.jobs")
