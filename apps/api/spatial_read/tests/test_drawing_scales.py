...
from apps.api.spatial_read.tests._read_helpers import make_stage
...
async def test_load_returns_only_floors_with_scale(db_session: AsyncSession, fake_clock: FakeClock) -> None:
    ...
    first, second, _third = stage.floors
    ...
    loaded = await SCALES[0].load(db_session, [floor.pk for floor in stage.floors])
...
async def test_load_uses_one_query_for_any_batch(db_session: AsyncSession, fake_clock: FakeClock) -> None:
    ...
    stage = await make_stage(db_session, floors=8)
    ...
    with count_sql() as small:
        await SCALES[0].load(db_session, [stage.floors[0].pk])
    with count_sql() as large:
        await SCALES[0].load(db_session, [floor.pk for floor in stage.floors])
