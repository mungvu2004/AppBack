...
from apps.api.spatial_read.tests._read_helpers import graph_path, make_stage
...
async def test_spatial_read_graph__C01(
    ...
    stage = await make_stage(db_session, floors=2, address="12 Nguyễn Huệ, Quận 1")
    first, second = stage.floors
    await _fill(db_session, first, 0, fake_clock)
    await db_session.commit()
    response = await api_client.get(graph_path(stage.project.id), headers=headers_of(stage.owner))
    ...
    assert graph["building"]["name"] == stage.project.name
...
async def test_spatial_read_graph__C01_empty_project(api_client: httpx.AsyncClient, db_session: AsyncSession) -> None:
    ...
    stage = await make_stage(db_session, floors=0)
    body = (await api_client.get(graph_path(stage.project.id), headers=headers_of(stage.owner))).json()
...
async def test_spatial_read_graph__C06(api_client: httpx.AsyncClient, db_session: AsyncSession) -> None:
    ...
    await db_session.commit()
    response = await api_client.get(graph_path(stage.project.id), headers=headers_of(outsider))
...
async def test_spatial_read_graph__C08(
    ...
    stage = await make_stage(db_session)
    await db_session.execute(update(Project).where(Project.id == stage.project.id).values(deleted_at=fake_clock.now()))
    await db_session.commit()
    response = await api_client.get(graph_path(stage.project.id), headers=headers_of(stage.owner))
    assert response.status_code == 404
    assert response.json()["resource"] == "project"
...
async def test_spatial_read_graph__C17(api_client: httpx.AsyncClient, db_session: AsyncSession) -> None:
    ...
    stage = await make_stage(db_session, address=None)
    body = (await api_client.get(graph_path(stage.project.id), headers=headers_of(stage.owner))).json()
...
async def test_spatial_read_graph_orders_equal_order_by_pk(
    ...
    stage = await make_stage(db_session, floors=3)
    await db_session.execute(update(FloorRow).where(FloorRow.project_id == stage.project.id).values(floor_order=0))
    await db_session.commit()
    body = (await api_client.get(graph_path(stage.project.id), headers=headers_of(stage.owner))).json()
    expected = [floor.level_id for floor in sorted(stage.floors, key=lambda row: row.pk)]
...
async def test_spatial_read_graph_skips_soft_deleted_floor(
    ...
    await db_session.commit()
    body = (await api_client.get(graph_path(stage.project.id), headers=headers_of(stage.owner))).json()
    assert [level["id"] for level in body["graph"]["levels"]] == [stage.floors[1].level_id]
    assert [item["floorId"] for item in body["floorRevisions"]] == [stage.floors[1].level_id]
...
async def test_spatial_read_graph_query_count_is_flat(
    ...
    small = await make_stage(db_session, floors=1)
    large = await make_stage(db_session, floors=8)
...
async def test_spatial_read_graph_level_matches_floors_list(
    ...
    await db_session.commit()
    listed = (await api_client.get(f"/api/projects/{stage.project.id}/floors", headers=headers_of(stage.owner))).json()[
        0
    ]
    level = (await api_client.get(graph_path(stage.project.id), headers=headers_of(stage.owner))).json()["graph"][
        "levels"
    ][0]
...
async def test_spatial_read_graph_building_reviewed_when_every_level_is(
    ...
    stage = await make_stage(db_session, floors=2)
    for index, floor in enumerate(stage.floors):
...
async def test_spatial_read_graph_never_writes(api_client: httpx.AsyncClient, db_session: AsyncSession) -> None:
    ...
    await api_client.get(graph_path(stage.project.id), headers=headers_of(stage.owner))
    await db_session.commit()
