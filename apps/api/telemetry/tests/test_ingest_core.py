...
def test_event_names_mirror_appfront(appfront_dir: Path) -> None:
    fe_names = read_ts_array(appfront_dir / "src/lib/telemetry/events.ts", "TELEMETRY_EVENT_NAMES")
    assert len(fe_names) == 12
    assert set(fe_names) == EVENT_NAMES


def test_error_kinds_mirror_appfront(appfront_dir: Path) -> None:
    fe_kinds = read_ts_array(appfront_dir / "src/lib/errors/kinds.ts", "APP_ERROR_KINDS")
...
def test_envelope_violations_are_422_with_field(overrides: dict[str, object], field: str) -> None:
    """SEC-05: `reason` không băm được (`[]`/`{}`) từng ném `TypeError` lọt ra 500 — nay 422."""
    with pytest.raises(AppError) as excinfo:
...
def test_reason_list_and_dict_do_not_crash_the_label_either() -> None:
    """`reason` sai kiểu vẫn tính được nhãn metric `invalid` (không đọc `label_reason` trước isinstance)."""
    bad_reasons: list[object] = [[], {}]
    for bad_reason in bad_reasons:
        with pytest.raises(AppError):
            ingest(_batch(reason=bad_reason), "application/json")
    assert 'appback_telemetry_batches_total{reason="invalid",result="rejected"} 2.0' in render()
...
def test_real_frontend_batch_is_accepted_and_counted() -> None:
    """C01 #37: 3 sự kiện hợp lệ, `droppedCount: 2` — khớp nguyên văn khối [8] của prompt."""
    events = [
    ...
    assert 'appback_telemetry_events_total{name="ai.finished"} 1.0' in text
    assert 'appback_telemetry_events_total{name="wall.edit"} 1.0' in text
...
def test_huge_integer_field_is_dropped_not_500() -> None:
    """SEC-05: số nguyên 400 chữ số từng ném `OverflowError` ở `math.isfinite` → 500. Nay 204, trường bị bỏ."""
    huge = int("7" * 400)
    events = [{"sequence": 0, "atMs": 0, "event": {"name": "wall.edit", "big": huge}}]
    ingest(_batch(events=events), "application/json")
    kept = ingest_module._sanitize_fields({"name": "wall.edit", "big": huge}, 16)
    assert "big" not in kept
    assert 'appback_telemetry_events_total{name="wall.edit"} 1.0' in render()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("okFlag", False, False),
        ("okCount", 42, 42),
        ("okDuration", 250.6, 251),
        ("okCode", "wall-01", "wall-01"),
        ("errorKind", "notFound", "notFound"),
        ("errorKind", "RateLimited", ingest_module._UNSET),
        ("fileName", "Bản vẽ nhà anh Ba.pdf", ingest_module._UNSET),
        ("email", "nguoi@example.com", ingest_module._UNSET),
        ("negative", -1, ingest_module._UNSET),
        ("tooBig", 1e9, ingest_module._UNSET),
        ("infinite", float("inf"), ingest_module._UNSET),
        ("array", [1, 2, 3], ingest_module._UNSET),
        ("mapping", {"a": 1}, ingest_module._UNSET),
    ],
    ids=[
        "bool-true",
        "bool-false",
...
def test_sanitize_value_output_per_type(key: str, value: object, expected: object) -> None:
    """TEST-01: khẳng định trực tiếp đầu ra `_sanitize_value` cho từng loại của [8]."""
    assert ingest_module._sanitize_value(key, value) == expected
...
def test_two_thousand_batches_do_not_drop_batches_series() -> None:
    ...
    rng = random.Random(7)  # noqa: S311 — dữ liệu test tất định qua seed, không phải bảo mật
...
def test_one_thousand_batches_respect_the_log_rate_limit(caplog: pytest.LogCaptureFixture) -> None:
    """1.000 lô x 20 sự kiện, đồng hồ giả ghim một phút: dòng log <= `TELEMETRY_LOG_MAX_PER_MIN`."""
    ...
    with caplog.at_level(logging.INFO, logger="apps.api.telemetry.ingest"):
        for _ in range(1_000):
            ingest(body, "application/json", clock=clock)
...
def test_log_bucket_resets_when_the_minute_rolls_over() -> None:
    """LOG-03: xô đọc phút từ `clock` tiêm được, không `time.time()` — qua phút là đặt lại."""
    clock = FakeClock(start=datetime(2026, 1, 1, tzinfo=UTC))
    limit = get_telemetry_settings().telemetry_log_max_per_min
    for _ in range(limit):
        assert ingest_module._allow_log(limit, clock)
    assert ingest_module._allow_log(limit, clock) is False
    clock.advance(timedelta(minutes=1))
    assert ingest_module._allow_log(limit, clock) is True
