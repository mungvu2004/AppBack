...
from apps.ml.runtime.gpu import GPU_LOCK_NAME, gpu_slot
...
def test_gpu_slot_release_survives_a_dead_redis(monkeypatch: pytest.MonkeyPatch) -> None:
    """Redis chết đúng lúc trả khoá: không che lỗi của thân khối, TTL dọn hộ."""
    with ephemeral_broker(monkeypatch) as admin:
        with gpu_slot(wait_s=0, ttl_ms=60_000, renew_every_ms=20_000) as slot:
            admin.shutdown(nosave=True)
        assert not slot.lost.is_set()
