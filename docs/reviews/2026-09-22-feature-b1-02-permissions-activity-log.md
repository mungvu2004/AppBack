...
|---|---|---|---|---|---|
| 1 | P2 | R-34 | Có 5 nợ trong báo cáo `bao-cao-B1-02.md` mà `DEBT.md` chưa có dòng nào: Role/PermissionKey khai hai nơi; khung không fail sớm với khoá `role:<…>` trên route BE-BIND; FE thiếu nhãn 26/27 `kind`; `record_activity` chưa kiểm `project_id`; chưa route nào gọi `record_activity`. Lý do: task gộp cấm sửa `DEBT.md` trên nhánh. | `DEBT.md` (main) | Commit thẳng lên `main` (ngoại lệ R-36) 5 dòng `NO-<nnn>` **trước khi merge** (R-38). |
...
## PHÁN QUYẾT: APPROVE
Không có P0/P1. Cổng đạt đủ 8 bước với mã thoát 0 trên đúng commit đầu nhánh. H3 đạt với 0 ô lệch. Mọi phần của B1-02 phủ 100% dòng và 100% nhánh. **Điều kiện trước khi merge (R-38):** commit thẳng lên `main` 5 dòng `NO-<nnn>` cho finding #1. Hai P3 (#2, #3) nằm trong số 5 dòng đó. Ba Nit không chặn merge. Merge bằng squash (nhánh chỉ một prompt), giữ trailer `Prompt: B1-02`.
